...
const SmartFarmChatbot = () => {
  const [messages, setMessages] = useState<{ role: string; content: string; time: string }[]>([
    { role: "assistant", content: "Hello! I'm your Smart Farm AI assistant. How can I help you today?", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, { role: "assistant", content: "Thank you for your question! This is a demo response. Connect to an AI backend for real answers.", time: t }]);
    }, 1000);
  };

  return (
    <DashboardLayout title="Smart Farm Chatbot (NLP)">
      <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-10rem)]">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-semibold text-foreground">Smart Farm Chatbot</h1>
            <p className="text-sm text-muted-foreground">NLP-powered assistant for all your farming questions</p>
          </div>
          <div className="flex rounded-full overflow-hidden border border-border">
            ...
            <button
              ...
              className={cn(
                "px-4 py-1.5 text-sm font-medium transition-colors",
                lang === "ar" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"
              )}
            >
              عربي
            </button>
        ...
        <div className="bg-card border border-border rounded-2xl flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-primary px-6 py-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-primary-foreground font-medium">AI Farm Assistant</p>
